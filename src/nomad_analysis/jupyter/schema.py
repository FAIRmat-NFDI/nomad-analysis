...
def write_header_cells(
    ...
    header_md_source = [
        ...
        '</div>\n',
        '\n',
        'This notebook was generated by and is linked to a NOMAD Jupyter Analysis\n'
        'ELN entry.\n',
        '\n',
        'Run the following cells to load the linked ELN entry into the variable:\n'
        '`analysis`. The analysis inputs can be accessed by `analysis.inputs`.\n',
        'You can add the analysis results to `analysis.results` and then\n',
        'run `analysis.save()` to update the state of the entry in the database.\n',
    ]
    cells.append(
